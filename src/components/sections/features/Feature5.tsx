...
const Feature5 = ({ isActive }: { isActive: boolean }) => {
    ...
    useEffect(() => {
        ...
        const runSequence = () => {
            ...
            // Phase 1 by default (Melissa Created)
            setPhase(1);
            next(2, 1000); // Melissa Added
            next(3, 2000); // Melissa Unassigned
            next(4, 3000); // Oscar Gilberto Completed (Top)

            // Hold the final state, then clear smoothly
            timeoutIds.push(setTimeout(() => {
                if (!isMounted) return;
                setPhase(1); // Reset to start state

                // Wait for exit animations to complete before restarting sequence logic if needed
                timeoutIds.push(setTimeout(() => {
